...
df.all.wide.info <- read_csv("~/MyDiv/MyDiv-litterfall-project/1-data/2-1-data-handling/2-1-1-Full-data-wideformat-MyDiv-litter-dryweight.csv")
...
ggplot(df.all.long, aes(x=tree_species_richness, y=dryweight, color=myc, fill = myc))+
  geom_point(shape =21, size = 1, alpha=0.5)+
  geom_smooth(method="lm", alpha=0.4)+
  labs(y=bquote("Leaf litter dryweight"~(g/m2)), 
       x = "Tree species richness")+
